...
namespace common
{
    ...
    auto AbstractReader::read(std::vector<char>& cBuf)
        -> int
    {
        return read(cBuf, 0, cBuf.size());
    }

    auto AbstractReader::markSupported() const
        -> bool
    {
        return false;
    }

    auto AbstractReader::ready() const
        -> bool
    {
        return false;
    }

    auto AbstractReader::skip(const size_t n)
        -> size_t
    {
        size_t skipped = 0;
        while (skipped < n)
