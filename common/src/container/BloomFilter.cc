    ...
    auto BloomFilter::operator==(const BloomFilter& f) const noexcept -> bool
    {
        if (this != &f)
        {
            return salt_count_ == f.salt_count_ && table_size_ == f.table_size_ && bit_table_.size() == f.bit_table_.
                size() && projected_element_count_ == f.projected_element_count_ && inserted_element_count_ == f.
                inserted_element_count_ && random_seed_ == f.random_seed_ && desired_false_positive_probability_ == f.
                desired_false_positive_probability_ && salt_ == f.salt_ && bit_table_ == f.bit_table_;
        }
        return true;
    }
