...
namespace common
{
    auto SystemInfo::GetCpuModelFromRegistry() noexcept -> std::string
    {
        ...
        if (const LONG regResult = RegOpenKeyExW(
            HKEY_LOCAL_MACHINE, L"HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0", 0, KEY_READ,
            &hKey); regResult == ERROR_SUCCESS)
        {
            wchar_t cpuName[256];
            ...
            {
                if (const int32_t len = WideCharToMultiByte(CP_UTF8, 0, cpuName, -1, nullptr, 0, nullptr, nullptr); len
                    > 0)
                {
    ...
    auto SystemInfo::GetMemoryDetails() noexcept -> std::string
    {
        ...
        const LONG resultReg = RegOpenKeyExW(
            HKEY_LOCAL_MACHINE,
            L"SYSTEM\\CurrentControlSet\\Control\\Class\\{4D36E965-" L"E325-11CE-BFC1-08002BE10318}", 0, KEY_READ,
            &hKey);
        ...
        if (resultReg == ERROR_SUCCESS)
        {
            ...
            if (RegQueryValueExW(hKey, L"DeviceDesc", nullptr, nullptr, reinterpret_cast<LPBYTE>(memInfo), &size) ==
                ERROR_SUCCESS)
            {
                if (const int32_t len = WideCharToMultiByte(CP_UTF8, 0, memInfo, -1, nullptr, 0, nullptr, nullptr); len
                    > 0)
                {
    ...
    auto SystemInfo::GetOSVersion() noexcept -> std::string
    {
        ...
        const LONG lResult = RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", 0,
                                           KEY_READ, &hKey);
        ...
        if (lResult == ERROR_SUCCESS)
        {
            ...
            if (RegQueryValueExW(hKey, L"ProductName", nullptr, nullptr, reinterpret_cast<LPBYTE>(osInfo), &size) ==
                ERROR_SUCCESS)
            {
                if (const int32_t len = WideCharToMultiByte(CP_UTF8, 0, osInfo, -1, nullptr, 0, nullptr, nullptr); len >
                    0)
                {
                    result.resize(len - 1); // len includes null terminator, so subtract 1
                ...
                if (RegQueryValueExW(hKey, L"CurrentBuildNumber", nullptr, nullptr, reinterpret_cast<LPBYTE>(buildNum),
                                     &size) == ERROR_SUCCESS)
                {
                    if (const int32_t buildLen = WideCharToMultiByte(
                        CP_UTF8, 0, buildNum, -1, nullptr, 0, nullptr, nullptr); buildLen > 0)
                    {
                        char buildStr[64];
    ...
    auto SystemInfo::GetMotherboardInfo() noexcept -> MotherboardInfo
    {
        ...
        if (result == ERROR_SUCCESS)
        {
            ...
            if (RegQueryValueExW(hKey, L"BaseBoardManufacturer", nullptr, nullptr, reinterpret_cast<LPBYTE>(buffer),
                                 &size) == ERROR_SUCCESS)
            {
                if (const int32_t len = WideCharToMultiByte(CP_UTF8, 0, buffer, -1, nullptr, 0, nullptr, nullptr); len >
                    0)
                {
            ...
            size = sizeof(buffer);
            if (RegQueryValueExW(hKey, L"BaseBoardProduct", nullptr, nullptr, reinterpret_cast<LPBYTE>(buffer), &size)
                == ERROR_SUCCESS)
            {
                if (const int32_t len = WideCharToMultiByte(CP_UTF8, 0, buffer, -1, nullptr, 0, nullptr, nullptr); len >
                    0)
        ...
        result = RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", 0, KEY_READ,
                               &hKey);
        ...
        if (result == ERROR_SUCCESS)
        {
            ...
            if (RegQueryValueExW(hKey, L"SystemSerialNumber", nullptr, nullptr, reinterpret_cast<LPBYTE>(buffer), &size)
                == ERROR_SUCCESS || RegQueryValueExW(hKey, L"ProductId", nullptr, nullptr,
                ...
                ERROR_SUCCESS)
            {
                ...
                if (RegOpenKeyExW(hKey, subKeyName, 0, KEY_READ, &hSubKey) == ERROR_SUCCESS)
                {
                    ...
                    if (RegQueryValueExW(hSubKey, L"DeviceDesc", nullptr, nullptr, reinterpret_cast<LPBYTE>(deviceDesc),
                                         &size) == ERROR_SUCCESS)
                    {
                        if (const int32_t len = WideCharToMultiByte(
                            CP_UTF8, 0, deviceDesc, -1, nullptr, 0, nullptr, nullptr); len > 0)
                        {
                            result.resize(len - 1); // len includes null terminator, so subtract 1
    ...
    auto SystemInfo::GetDiskDriveInfo() noexcept -> std::vector<std::string>
    {
        ...
        const LONG result = RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Services\\Disk\\Enum", 0,
                                          KEY_READ, &hKey);
        ...
        if (result == ERROR_SUCCESS)
        {
                                 ...
                                 reinterpret_cast<LPBYTE>(valueData), &valueDataSize) == ERROR_SUCCESS)
            {
                ...
                {
                    if (const int32_t len = WideCharToMultiByte(CP_UTF8, 0, valueData, -1, nullptr, 0, nullptr, nullptr)
                        ; len > 0)
                    {
    ...
    auto SystemInfo::GetBIOSInfo() noexcept -> std::vector<std::string>
    {
        ...
        const LONG result = RegOpenKeyExW(
            HKEY_LOCAL_MACHINE,
            L"SYSTEM\\CurrentControlSet\\Control\\Class" L"\\{4D36E972-E325-11CE-BFC1-08002BE10318}", 0, KEY_READ,
            &hKey);
                ...
                ERROR_SUCCESS)
            {
                if (iswdigit(subKeyName[0]))
                {
                    ...
                    if (RegOpenKeyExW(hKey, subKeyName, 0, KEY_READ, &hSubKey) == ERROR_SUCCESS)
                    {
                        ...
                        {
                            if (const int32_t len = WideCharToMultiByte(
                                CP_UTF8, 0, adapterName, -1, nullptr, 0, nullptr, nullptr); len > 0)
                            {
