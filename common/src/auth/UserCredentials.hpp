...
namespace common
{
    ...
    public:
        ...
        UserCredentials(std::string username, std::string hashed_password, std::string salt);

        /// @brief Get username
        /// @return Username string
        [[nodiscard]] auto get_username() const -> const std::string&;

        /// @brief Get hashed password
        /// @return Hashed password string
        [[nodiscard]] auto get_hashed_password() const -> const std::string&;

        /// @brief Get salt value
        /// @return Salt string used for hashing
        [[nodiscard]] auto get_salt() const -> const std::string&;

        /// @brief Get number of failed login attempts
        /// @return Count of failed attempts
        [[nodiscard]] auto get_failed_attempts() const -> size_t;

        /// @brief Increment failed login attempt counter
        auto increment_failed_attempts() -> void;

        /// @brief Reset failed attempt counter
        auto reset_failed_attempts() -> void;
