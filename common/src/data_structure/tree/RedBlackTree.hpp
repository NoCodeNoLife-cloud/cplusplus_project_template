...
namespace fox
{
    ...
    public:
        ...
        [[nodiscard]] auto getRoot() const noexcept
            -> std::shared_ptr<RedBlackTreeNode<T>>;
    ...
    private:
        ...
        auto leftRotate(std::shared_ptr<RedBlackTreeNode<T>> node)
            -> void;

        /// @brief Perform a right rotation on the given node
        /// @param node The node to rotate
        auto rightRotate(std::shared_ptr<RedBlackTreeNode<T>> node)
            -> void;

        /// @brief Fix the red-black tree after insertion
        /// @param node The node that was inserted
        auto fixInsert(std::shared_ptr<RedBlackTreeNode<T>> node)
            -> void;
    };
    ...
    template <typename T>
    auto RedBlackTree<T>::insert(const T& value)
        -> void
    {
    ...
    template <typename T>
    auto RedBlackTree<T>::getRoot() const noexcept
        -> std::shared_ptr<RedBlackTreeNode<T>>
    {
        return root_;
    }
