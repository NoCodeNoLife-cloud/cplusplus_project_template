...
namespace common
{
    ...
    public:
        ...
        [[nodiscard]] auto format(const std::tm& date) const
            -> std::string;
        ...
        [[nodiscard]] auto parse(const std::string& dateStr) const
            -> std::tm;
        ...
        [[nodiscard]] auto equals(const SimpleDateFormatter& other) const noexcept
            -> bool;
    ...
    private:
        ...
        /// @throws std::invalid_argument If the pattern is empty
        static auto validatePattern(const std::string& pat)
            -> void;
    };
}
