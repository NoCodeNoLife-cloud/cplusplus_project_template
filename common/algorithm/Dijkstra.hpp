...
template <typename GraphType, typename DistanceType = int32_t> class Dijkstra
{
  public:
...
template <typename GraphType, typename DistanceType>
template <typename DistanceFunc>
auto Dijkstra<GraphType, DistanceType>::compute(std::decay_t<decltype(std::declval<GraphType>().getNodeCount())> start,
