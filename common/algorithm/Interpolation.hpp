...
inline auto Interpolation::linear(const std::vector<std::pair<double, double>>& dataPoints, const double x) -> double {
...
inline auto Interpolation::nearest(const std::vector<std::pair<double, double>>& dataPoints, const double x) -> double {
...
inline void Interpolation::validateDataPoints(const std::vector<std::pair<double, double>>& dataPoints, size_t minSize) {
  if (dataPoints.size() < minSize) {
    throw std::invalid_argument("At least " + std::to_string(minSize) + " data point(s) required for this interpolation method.");
  }
...
inline size_t Interpolation::findLowerBound(const std::vector<std::pair<double, double>>& dataPoints, const double x) {
  ...
  auto it = std::lower_bound(dataPoints.begin(), dataPoints.end(), x, [](const std::pair<double, double>& a, const double b) { return a.first < b; });
...
inline size_t Interpolation::findClosestPoint(const std::vector<std::pair<double, double>>& dataPoints, const double x) {
  ...
  const auto it = std::lower_bound(dataPoints.begin(), dataPoints.end(), x, [](const std::pair<double, double>& a, const double b) { return a.first < b; });
  ...
  return distToPrev <= distToNext ? std::distance(dataPoints.begin(), prevIt) : std::distance(dataPoints.begin(), nextIt);
}
}  // namespace fox
