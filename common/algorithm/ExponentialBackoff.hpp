...
class ExponentialBackoff
{
  public:
    ...
    ExponentialBackoff(uint32_t max_retries, double initial_delay, double multiplier, double min_delay = 0.0,
                       double max_cap = std::numeric_limits<double>::infinity(), uint32_t seed = std::random_device{}(),
...
inline ExponentialBackoff::ExponentialBackoff(const uint32_t max_retries, const double initial_delay,
                                              const double multiplier, const double min_delay, const double max_cap,
                                              const uint32_t seed, const bool thread_safe)
    : max_retries_(max_retries), initial_delay_(initial_delay), multiplier_(multiplier), min_delay_(min_delay),
      max_cap_(max_cap), rng_(seed), thread_safe_(thread_safe)
{
    ...
    if (max_cap < 0)
        throw std::invalid_argument("Max cap must be non-negative.");
    if (min_delay > max_cap)
        throw std::invalid_argument("Minimum delay must not exceed max cap.");
    if (initial_delay < min_delay)
        throw std::invalid_argument("Initial delay must be greater than or equal to min_delay.");
}
...
inline auto ExponentialBackoff::getMaxRetries() const -> uint32_t
{
    return max_retries_;
}
