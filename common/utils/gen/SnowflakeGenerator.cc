...
SnowflakeGenerator::SnowflakeGenerator(const int16_t machine_id,
  ...
  if (machine_id < 0 ||
      machine_id > static_cast<int64_t>(SnowflakeOption::max_machine_id_)) {
    throw std::invalid_argument("Machine ID out of range (0-31)");
  }
  if (datacenter_id < 0 ||
      datacenter_id >
          static_cast<int64_t>(SnowflakeOption::max_datacenter_id_)) {
...
auto SnowflakeGenerator::NextId() -> int64_t {
  ...
  if (timestamp == last_timestamp_) {
    sequence_ =
        sequence_ + 1 & static_cast<int64_t>(SnowflakeOption::max_sequence_);
    if (sequence_ == 0) {
  ...
  return timestamp << (static_cast<int64_t>(SnowflakeOption::machine_bits_) +
         ...
         static_cast<int64_t>(machine_id_)
             << static_cast<int64_t>(SnowflakeOption::sequence_bits_) |
         sequence_;
}
...
auto SnowflakeGenerator::TilNextMillis(const int64_t last_timestamp)
    -> int64_t {
  int64_t timestamp = GetCurrentTimestamp();
