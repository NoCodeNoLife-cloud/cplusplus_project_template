...
class Date {
 public:
  ...
  Date(const int32_t year, const int32_t month, const int32_t day, const int32_t hours, const int32_t minutes, const int32_t seconds) {
  ...
  explicit Date(const int64_t timestamp) { time_point_ = std::chrono::system_clock::time_point(std::chrono::milliseconds(timestamp)); }
  ~Date() = default;

  /// @brief Creates and returns a copy of this object.
  [[nodiscard]] auto clone() const -> Date { return Date(getTime()); }

  /// @brief Compares this date to the specified date.
  [[nodiscard]] auto equals(const Date& other) const -> bool { return time_point_ == other.time_point_; }

  /// @brief Tests if this date is after the specified date.
  [[nodiscard]] auto after(const Date& other) const -> bool { return time_point_ > other.time_point_; }

  /// @brief Tests if this date is before the specified date.
  [[nodiscard]] auto before(const Date& other) const -> bool { return time_point_ < other.time_point_; }

  /// @brief Returns the number of milliseconds since January 1, 1970, 00:00:00 GMT.
  [[nodiscard]] auto getTime() const -> int64_t { return std::chrono::duration_cast<std::chrono::milliseconds>(time_point_.time_since_epoch()).count(); }

  /// @brief Returns the year represented by this date.
  [[nodiscard]] auto getYear() const -> int32_t { return toTm().tm_year + 1900; }

  /// @brief Returns the month represented by this date.
  [[nodiscard]] auto getMonth() const -> int32_t { return toTm().tm_mon + 1; }

  /// @brief Returns the day of the month represented by this date.
  [[nodiscard]] auto getDay() const -> int32_t { return toTm().tm_mday; }
  ...
  [[nodiscard]] auto hashCode() const -> size_t { return std::hash<int64_t>()(getTime()); }
