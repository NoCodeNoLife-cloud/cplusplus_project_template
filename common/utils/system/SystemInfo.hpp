...
namespace fox
{
    ...
    inline auto SystemInfo::GetCpuModelFromRegistry() noexcept -> std::string
    {
        HKEY hKey;
    ...
    inline auto SystemInfo::GetMemoryDetails() noexcept -> std::string
    {
        HKEY hKey;
        std::string result;
    ...
    inline auto SystemInfo::GetOSVersion() noexcept -> std::string
    {
        HKEY hKey;
        std::string result;

        const LONG lResult =
            RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", 0, KEY_READ,
                          &hKey);
    ...
    inline auto SystemInfo::GetMotherboardInfo() noexcept -> MotherboardInfo
    {
    ...
    inline auto SystemInfo::GetGraphicsCardInfo() noexcept -> std::string
    {
        HKEY hKey;
        std::string result;

        const LONG resultReg = RegOpenKeyExW(
            HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers\\Devices", 0, KEY_READ,
            &hKey);
    ...
    inline auto SystemInfo::GetDiskDriveInfo() noexcept -> std::vector<std::string>
    {
        std::vector<std::string> drives;
        HKEY hKey;

        const LONG result =
            RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Services\\Disk\\Enum", 0, KEY_READ,
                          &hKey);
    ...
    inline auto SystemInfo::GetBIOSInfo() noexcept -> std::vector<std::string>
    {
