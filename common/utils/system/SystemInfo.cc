...
namespace common
{
    auto SystemInfo::GetCpuModelFromRegistry() -> std::string
    {
        ...
        if (LONG regResult = RegOpenKeyExW(HKEY_LOCAL_MACHINE,
                                           L"HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0",
                                           0, KEY_READ, &hKey); regResult == ERROR_SUCCESS)
        {
            wchar_t cpuName[256];
    ...
    auto SystemInfo::GetMemoryDetails() -> std::string
    {
        HKEY hKey;
        std::string result;

        const LONG resultReg = RegOpenKeyExW(HKEY_LOCAL_MACHINE,
                                             L"SYSTEM\\CurrentControlSet\\Control\\Class\\{4D36E965-E325-11CE-BFC1-08002BE10318}",
                                             0, KEY_READ, &hKey);
    ...
    auto SystemInfo::GetOSVersion() -> std::string
    {
        HKEY hKey;
        std::string result;

        const LONG lResult = RegOpenKeyExW(HKEY_LOCAL_MACHINE,
    ...
    auto SystemInfo::GetMotherboardInfo() -> MotherboardInfo
    {
        ...
        if (result == ERROR_SUCCESS)
        {
            ...
            if (RegQueryValueExW(hKey, L"SystemSerialNumber", nullptr, nullptr, reinterpret_cast<LPBYTE>(buffer), &size) == ERROR_SUCCESS ||
                RegQueryValueExW(hKey, L"ProductId", nullptr, nullptr, reinterpret_cast<LPBYTE>(buffer), &size) == ERROR_SUCCESS)
            {
                if (const int len = WideCharToMultiByte(CP_UTF8, 0, buffer, -1, nullptr, 0, nullptr, nullptr); len > 0)
                {
    ...
    auto SystemInfo::GetGraphicsCardInfo() -> std::string
    {
        HKEY hKey;
        std::string result;

        const LONG resultReg = RegOpenKeyExW(HKEY_LOCAL_MACHINE,
                                             L"SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers\\Devices",
                                             0, KEY_READ, &hKey);
    ...
    auto SystemInfo::GetDiskDriveInfo() -> std::vector<std::string>
    {
        ...
        if (result == ERROR_SUCCESS)
        {
            ...
            while (RegEnumValueW(hKey, index++, valueName, &valueNameSize,
                                 nullptr, nullptr, reinterpret_cast<LPBYTE>(valueData), &valueDataSize) == ERROR_SUCCESS)
            {
    ...
    auto SystemInfo::GetBIOSInfo() -> std::vector<std::string>
    {
