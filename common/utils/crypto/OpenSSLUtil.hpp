...
class OpenSSLUtil {
 public:
  ...
  static auto deriveKey(const std::string& password, unsigned char key[32], unsigned char salt[16]) -> void { EVP_BytesToKey(EVP_aes_256_cbc(), EVP_sha1(), salt, reinterpret_cast<const unsigned char*>(password.c_str()), static_cast<int32_t>(password.size()), 1, key, nullptr); }
  ...
  static auto encryptAES256CBC(const std::string& plaintext, const std::string& password, unsigned char salt[16]) -> std::vector<unsigned char> {
  ...
  static auto decryptAES256CBC(const std::vector<unsigned char>& ciphertext, const std::string& password, unsigned char salt[16]) -> std::string {
    if (ciphertext.size() < AES_BLOCK_SIZE) throw std::runtime_error("Invalid ciphertext length");
    ...
    EVP_DecryptUpdate(ctx, plaintext.data(), &len, ciphertext.data() + AES_BLOCK_SIZE, static_cast<int32_t>(ciphertext.size() - AES_BLOCK_SIZE));
    plaintext_len += len;
