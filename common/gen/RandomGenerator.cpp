...
{
    RandomGenerator::RandomGenerator(): generator(std::random_device{}()), realDist(0.0, 1.0), normalDist(0.0, 1.0) {}

    RandomGenerator::RandomGenerator(const int64_t seed): generator(seed), realDist(0.0, 1.0), normalDist(0.0, 1.0) {}
    ...
    {
        std::uniform_int_distribution<unsigned int32_t>
        dist(0, 255);
    ...
    auto RandomGenerator::next(const int32_t bits) -> int32_t
    {
