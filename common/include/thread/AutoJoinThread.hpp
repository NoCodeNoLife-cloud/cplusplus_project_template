...
namespace fox
{
    ...
    inline AutoJoinThread::AutoJoinThread(AutoJoinThread&& other) noexcept : thread_(std::move(other.thread_))
    {
    }
    ...
    inline AutoJoinThread::~AutoJoinThread()
    {
        if (thread_.joinable())
        {
            thread_.join();
    ...
    inline auto AutoJoinThread::detach() -> void
    {
        if (thread_.joinable())
        {
            thread_.detach();
    ...
    inline auto AutoJoinThread::join() -> void
    {
        if (thread_.joinable())
        {
            thread_.join();
    ...
    inline auto AutoJoinThread::joinable() const noexcept -> bool
    {
        return thread_.joinable();
    }

    inline auto AutoJoinThread::swap(AutoJoinThread& other) noexcept -> void
    {
        thread_.swap(other.thread_);
    }

    inline auto AutoJoinThread::native_handle() -> std::thread::native_handle_type
    {
    ...
    /// @param other The source object to move from
    /// @return Reference to this object
    inline auto AutoJoinThread::operator=(AutoJoinThread&& other) noexcept -> AutoJoinThread&
    {
        if (this != &other)
        {
            if (thread_.joinable())
