...
class BufferedInputStream final : public FilterInputStream {
 public:
  explicit BufferedInputStream(std::unique_ptr<AbstractInputStream> in) : BufferedInputStream(std::move(in), DEFAULT_BUFFER_SIZE) {}
  BufferedInputStream(std::unique_ptr<AbstractInputStream> in, const int32_t size) : FilterInputStream(std::move(in)), buf_(size) {
    if (!input_stream_) {
  ...
  [[nodiscard]] auto available() const -> size_t { return count_ - pos_ + input_stream_->available(); }
