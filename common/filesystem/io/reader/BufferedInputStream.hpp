...
inline auto BufferedInputStream::available() const -> size_t { return count_ - pos_ + input_stream_->available(); }
...
inline void BufferedInputStream::mark(int32_t readLimit) {
  mark_limit_ = readLimit;
  input_stream_->mark(readLimit);
  mark_pos_ = pos_;
}

inline bool BufferedInputStream::markSupported() const { return true; }
