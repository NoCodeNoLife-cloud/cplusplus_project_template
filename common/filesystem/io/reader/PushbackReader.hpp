...
class PushbackReader final : public FilterReader {
 public:
  ...
  /// If the pushback buffer is not empty, characters from the pushback buffer are returned.
  /// Otherwise, characters from the underlying input stream are returned.
...
inline void PushbackReader::mark(size_t readAheadLimit) { throw std::runtime_error("mark() not supported."); }

inline bool PushbackReader::markSupported() const { return false; }
...
inline void PushbackReader::reset() { throw std::runtime_error("reset() not supported."); }
...
inline auto PushbackReader::unread(const std::vector<char>& cbuf) -> void { unread(cbuf, 0, cbuf.size()); }
