...
class StringReader final : public AbstractReader {
 public:
  ...
  /// @return The total number of characters read into the buffer, or -1 if there is no more data because the end of the string has been reached.
...
inline StringReader::StringReader(std::string s) : source_(std::move(s)), position_(0), mark_position_(0), mark_set_(false) {}
...
inline bool StringReader::markSupported() const { return true; }
...
inline bool StringReader::ready() const { return position_ < source_.size(); }
