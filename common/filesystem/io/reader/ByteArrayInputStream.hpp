...
inline auto ByteArrayInputStream::available() -> size_t { return buffer_.size() - pos_; }

inline void ByteArrayInputStream::reset() { pos_ = mark_position_; }

inline void ByteArrayInputStream::mark(const int32_t readLimit) { mark_position_ = std::min(static_cast<size_t>(readLimit), buffer_.size()); }

inline bool ByteArrayInputStream::markSupported() const { return true; }
