...
inline BufferedReader::BufferedReader(std::unique_ptr<AbstractReader> reader, const int32_t size) : reader_(std::move(reader)), buffer_size_(size) {
  if (size <= 0) {
...
inline auto BufferedReader::close() -> void { reader_->close(); }
...
inline bool BufferedReader::markSupported() const { return true; }
...
inline bool BufferedReader::ready() const { return reader_->ready(); }
