...
InputStreamReader::InputStreamReader(std::shared_ptr<AbstractReader> input,
  ...
  if (charsetName != "UTF-8") {
    throw std::invalid_argument(
        "Only UTF-8 charset is supported in this implementation");
  }
}
...
auto InputStreamReader::read(std::vector<char>& cBuf, const size_t off,
  ...
  try {
    auto chars =
        converter_.from_bytes(byteBuffer.data(), byteBuffer.data() + bytesRead);
    const size_t charsToCopy = std::min(chars.size(), len);
    std::copy_n(
        chars.begin(), charsToCopy,
