...
inline auto AbstractInputStream::mark(int32_t readLimit) -> void { throw std::runtime_error("mark not supported"); }

inline auto AbstractInputStream::markSupported() const -> bool { return false; }

inline auto AbstractInputStream::read(std::vector<std::byte>& buffer) -> size_t { return read(buffer, 0, buffer.size()); }

inline auto AbstractInputStream::read(std::vector<std::byte>& buffer, size_t offset, size_t len) -> size_t {
...
inline auto AbstractInputStream::reset() -> void { throw std::runtime_error("reset not supported"); }

inline auto AbstractInputStream::skip(size_t n) -> size_t {
  size_t skipped = 0;
