...
class AbstractInputStream : public ICloseable {
 public:
  ...
  virtual auto mark(int32_t readLimit) -> void { throw std::runtime_error("mark not supported"); }
  ...
  virtual auto read(std::vector<std::byte>& buffer) -> size_t { return read(buffer, 0, static_cast<int32_t>(buffer.size())); }
  ...
  virtual auto reset() -> void { throw std::runtime_error("reset not supported"); }
