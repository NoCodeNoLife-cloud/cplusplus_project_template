...
class BufferedOutputStream final : public FilterOutputStream {
 public:
  ...
  BufferedOutputStream(std::unique_ptr<AbstractOutputStream> out, const size_t size) : FilterOutputStream(std::move(out)), bufferSize_(size), buffer_(size), buffer_position_(0) {
    if (!output_stream_) {
  ...
  auto close() -> void override { output_stream_->close(); }
