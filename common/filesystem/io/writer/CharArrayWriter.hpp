...
class CharArrayWriter final : public AbstractWriter, IAppendable<CharArrayWriter> {
 public:
  ...
  auto writeTo(AbstractWriter& out) const -> void { out.write(buf_, 0, count_); }
  ...
  [[nodiscard]] auto toCharArray() const -> std::vector<char> { return {buf_.begin(), buf_.begin() + static_cast<std::vector<char>::difference_type>(count_)}; }
  ...
  [[nodiscard]] auto toString() const -> std::string override { return {buf_.begin(), buf_.begin() + static_cast<std::vector<char>::difference_type>(count_)}; }
