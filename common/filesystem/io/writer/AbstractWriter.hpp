...
namespace common {
class AbstractWriter : public ICloseable,
                       public IFlushable,
                       IAppendable<AbstractWriter> {
 public:
  AbstractWriter();
  ...
  auto append(const std::string& csq, size_t start, size_t end)
      -> AbstractWriter& override;
  virtual auto write(char c) -> void;
  virtual auto write(const std::vector<char>& cBuf) -> void;
  virtual auto write(const std::vector<char>& cBuf, size_t off, size_t len)
      -> void = 0;
