...
namespace fox
{
    ...
    public:
        ...
        [[nodiscard]] auto mkdirs(const bool exist_ok = false) const -> bool
        {
            try
            {
                if (exist_ok)
        ...
        [[nodiscard]] auto listDir(const bool recursive) const -> std::vector<std::filesystem::directory_entry>
        {
        ...
        /// @param recursive Whether to list subdirectories recursively
        /// @return A vector of directory entries
        [[nodiscard]] static auto listDir(const std::filesystem::path& dir_path, const bool recursive)
        ...
        [[nodiscard]] auto listEntries(
            const bool recursive = false) const -> std::vector<std::filesystem::directory_entry>
        {
            return listDir(dir_path_, recursive);
        }
