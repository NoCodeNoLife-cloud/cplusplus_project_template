...
namespace fox
{
    ...
    template <typename T>
    inline auto YamlObjectSerializer<T>::serialize(const T& obj, const std::string& filename) -> void
    {
        if (filename.empty())
        {
    ...
    template <typename T>
    [[nodiscard]] inline auto YamlObjectSerializer<T>::deserialize(const std::string& filename) -> T
    {
        if (filename.empty())
        {
