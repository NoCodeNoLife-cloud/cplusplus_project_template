...
template <typename T> class YamlObjectSerializer
{
  public:
...
template <typename T> auto YamlObjectSerializer<T>::serialize(const T &obj, const std::string &filename) -> void
{
    if (filename.empty())
    {
...
template <typename T> auto YamlObjectSerializer<T>::deserialize(const std::string &filename) -> T
{
