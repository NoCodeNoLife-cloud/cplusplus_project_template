...
class JsonObjectSerializer abstract {
 public:
  ...
  static auto saveObjectToJsonFile(const T& entity, const std::string& filename)
      -> void;
  template <DerivedFromJsonSerializable T>
  ...
  static auto serializeField(rapidjson::Writer<rapidjson::StringBuffer>& writer,
                             const char* key, const std::string& value) -> void;
  static auto serializeField(rapidjson::Writer<rapidjson::StringBuffer>& writer,
                             const char* key, int32_t value) -> void;
  static auto serializeField(rapidjson::Writer<rapidjson::StringBuffer>& writer,
                             const char* key, double value) -> void;
  static auto serializeField(rapidjson::Writer<rapidjson::StringBuffer>& writer,
                             const char* key, bool value) -> void;
};

template <DerivedFromJsonSerializable T>
auto JsonObjectSerializer::saveObjectToJsonFile(const T& entity,
...
template <DerivedFromJsonSerializable T>
auto JsonObjectSerializer::loadObjectFromJsonFile(const std::string& filename)
    -> T {
  T entity{};
  ...
  const std::string jsonStr{std::istreambuf_iterator(ifs),
