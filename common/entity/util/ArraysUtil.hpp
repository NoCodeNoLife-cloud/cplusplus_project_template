...
public:
  ...
  template <typename T>
  static auto binarySearch(const T* array, size_t size, const T& key) -> int {
  ...
  /// \returns The index of the key within the array if found; otherwise, -1.
  template <typename T>
  static auto binarySearch(const T* array, size_t fromIndex, size_t toIndex, const T& key) -> int {
  ...
  template <typename T>
  static auto copyOf(const T* original, const size_t originalSize, size_t newLength) -> std::vector<T> {
  ...
  template <typename T>
  static auto equals(const T* a, size_t sizeA, const T* b, const size_t sizeB) -> bool {
  ...
  template <typename T>
  static auto toString(const T* array, const size_t size) -> std::string {
