...
class BinarySearchTree {
 ...
 private:
  ...
  auto insertRecursive(std::shared_ptr<TreeNode<T>> node, T value)
      -> std::shared_ptr<TreeNode<T>>;
  auto findRecursive(const std::shared_ptr<TreeNode<T>>& node, T value) const
      -> bool;
  auto removeRecursive(std::shared_ptr<TreeNode<T>> node, T value)
      -> std::shared_ptr<TreeNode<T>>;
  auto minValueNode(std::shared_ptr<TreeNode<T>> node)
      -> std::shared_ptr<TreeNode<T>>;
...
template <typename T>
auto BinarySearchTree<T>::findRecursive(
    const std::shared_ptr<TreeNode<T>>& node, T value) const -> bool {
...
template <typename T>
auto BinarySearchTree<T>::inorderTraversalRecursive(
    const std::shared_ptr<TreeNode<T>>& node) const -> void {
  if (!node) return;
