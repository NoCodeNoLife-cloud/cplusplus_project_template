...
namespace fox
{
template <typename T> struct ReflectTraits;
...
class ClassUtil
{
  public:
    ...
    template <typename T> static auto getTypeId(const T &t) -> std::string;
    ...
    template <typename T> static auto getTypeIdWithCvr(const T &t) -> std::string;

    /// @brief Get the type id by class type.
    /// @tparam T Type of the class.
    /// @return std::string The pretty name_ of the type.
    template <typename T> static auto getTypeIdByClass() -> std::string;

    /// @brief Get the type id with CVR by class type.
    /// @tparam T Type of the class.
    /// @param t Placeholder parameter, not used in implementation.
    /// @return std::string The pretty name_ of the type with CVR.
    template <typename T> static auto getTypeIdWithCvrByClass(const std::string &t) -> std::string;
    ...
    template <typename T> static std::unordered_map<std::string, std::string> getFields(const T &obj);
  ...
  private:
    ...
    template <typename T, typename M> static auto invokeHelper(const T &obj, M member) -> decltype(obj.*member);
...
template <typename T> auto ClassUtil::getTypeId(const T &) -> std::string
{
    return boost::typeindex::type_id<T>().pretty_name();
}

template <typename T> auto ClassUtil::getTypeIdWithCvr(const T &) -> std::string
{
    return boost::typeindex::type_id_with_cvr<T>().pretty_name();
}

template <typename T> auto ClassUtil::getTypeIdByClass() -> std::string
{
    return boost::typeindex::type_id<T>().pretty_name();
}

template <typename T> auto ClassUtil::getTypeIdWithCvrByClass(const std::string &) -> std::string
{
    return boost::typeindex::type_id_with_cvr<T>().pretty_name();
}

template <typename T> std::unordered_map<std::string, std::string> ClassUtil::getFields(const T &obj)
{
...
template <typename T, typename M> auto ClassUtil::invokeHelper(const T &obj, M member) -> decltype(obj.*member)
{
    return obj.*member;
}
