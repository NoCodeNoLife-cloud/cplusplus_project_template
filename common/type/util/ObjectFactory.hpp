...
template <typename T> class ObjectFactory : public service::IStartupTask
{
  public:
    ...
    template <typename V, typename... Args> static void registerType(const std::string &type_name, Args &&...args);
...
template <typename T> std::unique_ptr<T> ObjectFactory<T>::createObject(const std::string &type_name)
{
...
template <typename T> auto ObjectFactory<T>::execute() -> bool
{
    registerAll();
    return true;
}
