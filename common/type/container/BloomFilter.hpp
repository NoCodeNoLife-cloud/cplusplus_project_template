...
class BloomFilter final {
 ...
 public:
  ...
  explicit BloomFilter(const BloomParameters& p) : projected_element_count_(p.projected_element_count), random_seed_(p.random_seed * 0xA5A5A5A5 + 1), desired_false_positive_probability_(p.false_positive_probability) {
  ...
  auto operator!=(const BloomFilter& f) const -> bool { return !operator==(f); }
  ...
  auto operator!() const -> bool { return 0 == table_size_; }
  ...
  auto insert(const std::string& key) -> void { insert(reinterpret_cast<const unsigned char*>(key.data()), key.size()); }

  /// @brief Inserts a key into the Bloom filter.
  /// @param data Pointer to the beginning of the key.
  /// @param length Length of the key.
  auto insert(const char* data, const std::size_t& length) -> void { insert(reinterpret_cast<const unsigned char*>(data), length); }
  ...
  /// @return True if the key is present, false otherwise.
  [[nodiscard]] auto contains(const std::string& key) const -> bool { return contains(reinterpret_cast<const unsigned char*>(key.c_str()), key.size()); }

  /// @brief Checks if a key is present in the Bloom filter.
  /// @param data Pointer to the beginning of the key.
  /// @param length Length of the key.
  /// @return True if the key is present, false otherwise.
  auto contains(const char* data, const std::size_t& length) const -> bool { return contains(reinterpret_cast<const unsigned char*>(data), length); }
  ...
  [[nodiscard]] auto size() const -> uint64_t { return table_size_; }

  /// @brief Gets the number of elements inserted into the Bloom filter.
  /// @return The number of elements inserted.
  [[nodiscard]] auto element_count() const -> uint64_t { return inserted_element_count_; }

  /// @brief Gets the effective false positive probability.
  /// @return The effective false positive probability.
  [[nodiscard]] auto effective_fpp() const -> double { return std::pow(1.0 - std::exp(-1.0 * static_cast<double>(salt_.size()) * static_cast<double>(inserted_element_count_) / static_cast<double>(size())), 1.0 * static_cast<double>(salt_.size())); }
  ...
  [[nodiscard]] auto table() const -> const cell_type_* { return bit_table_.data(); }

  /// @brief Gets the number of hash functions used.
  /// @return The number of hash functions.
  [[nodiscard]] auto hash_count() const -> std::size_t { return salt_.size(); }
