...
namespace common
{
    ...
    BloomFilter::BloomFilter(const BloomParameters& p): projected_element_count_(p.projected_element_count),
    ...
    auto BloomFilter::operator!() const -> bool
    {
    ...
    {
        return std::pow(1.0 - std::exp(-1.0 * static_cast<double>(salt_.size()) * static_cast<double>(inserted_element_count_) / static_cast<double>(size())), 1.0 * static_cast<double>(salt_.size()));
    }
    ...
    auto BloomFilter::hash_count() const -> std::size_t
    {
        return salt_.size();
    }
