...
auto BloomFilter::insert(const InputIterator begin, const InputIterator end)
    -> void {
...
auto BloomFilter::contains(const unsigned char* key_begin,
                           const std::size_t length) const -> bool {
  std::size_t bit_index = 0;
  std::size_t bit = 0;
...
auto BloomFilter::contains(const char* data, const std::size_t& length) const
    -> bool {
  return contains(reinterpret_cast<const unsigned char*>(data), length);
}

template <typename InputIterator>
auto BloomFilter::contains_all(const InputIterator begin,
                               const InputIterator end) const -> InputIterator {
  InputIterator itr = begin;
...
template <typename InputIterator>
auto BloomFilter::contains_none(const InputIterator begin,
...
auto BloomFilter::effective_fpp() const -> double {
  return std::pow(1.0 - std::exp(-1.0 * static_cast<double>(salt_.size()) *
                                 static_cast<double>(inserted_element_count_) /
  ...
  if (salt_count_ <= pre_def_salt_count) {
    ...
    for (std::size_t i = 0; i < salt_.size(); ++i) {
      salt_[i] = salt_[i] * salt_[(i + 3) % salt_.size()] +
                 static_cast<bloom_type_>(random_seed_);
    }
  } else {
...
auto BloomFilter::hash_ap(const unsigned char* begin,
                          std::size_t remaining_length, bloom_type_ hash)
    -> bloom_type_ {
