...
template <typename Derived> class IAop
{
  public:
    ...
    template <typename Func, typename... Args> auto exec(Func &&func, Args &&...args) -> decltype(auto);
  ...
  private:
    ...
    template <typename T> auto handleResult(T &&result) -> decltype(auto);
};
...
template <typename Derived> template <typename T> auto IAop<Derived>::handleResult(T &&result) -> decltype(auto)
{
