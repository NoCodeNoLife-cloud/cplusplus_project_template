...
namespace framework::entity::data_structure::tree {
    ...
    template <typename T>
    auto BinarySearchTree<T>::insert(T value) -> void { root = insertRecursive(root, value); }
    ...
    template <typename T>
    auto BinarySearchTree<T>::find(T value) const -> bool { return findRecursive(root, value); }

    template <typename T>
    auto BinarySearchTree<T>::findRecursive(const std::shared_ptr<TreeNode<T>>& node, T value) const -> bool {
        if (!node) return false;
        if (value == node->data) return true;
        else if (value < node->data) return findRecursive(node->left, value);
        else return findRecursive(node->right, value);
    }

    template <typename T>
    auto BinarySearchTree<T>::remove(T value) -> void { root = removeRecursive(root, value); }
