...
namespace framework::entity::data_structure::tree {
    ...
    auto RedBlackTree<T>::insert(const T& value) -> void {
        ...
        {
            parent = current;
            if (value < current->getData()) { current = current->getLeft(); }
            else { current = current->getRight(); }
        }
        node->setParent(parent);
        if (value < parent->getData()) { parent->setLeft(node); }
    ...
    auto RedBlackTree<T>::leftRotate(std::shared_ptr<RedBlackTreeNode<T>> node) -> void {
        ...
        if (!node->getParent()) { root = rightChild; }
        else if (node == node->getParent()->getLeft()) { node->getParent()->setLeft(rightChild); }
    ...
    auto RedBlackTree<T>::rightRotate(std::shared_ptr<RedBlackTreeNode<T>> node) -> void {
        ...
        if (!node->getParent()) { root = leftChild; }
        else if (node == node->getParent()->getLeft()) { node->getParent()->setLeft(leftChild); }
        else { node->getParent()->setRight(leftChild); }
        ...
        while (node != root && node->getParent() && node->getParent()->getColor() == graphics::models::Color::Red)
        {
            if (node->getParent() == node->getParent()->getParent()->getLeft())
            {
                ...
                else
                {
                    ...
                    node->getParent()->setColor(graphics::models::Color::Black);
                    if (node->getParent()->getParent()) { node->getParent()->getParent()->setColor(graphics::models::Color::Red); }
