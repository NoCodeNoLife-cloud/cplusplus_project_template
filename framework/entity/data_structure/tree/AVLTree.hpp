...
namespace framework::entity::data_structure::tree {
    ...
    template <typename T>
    auto AVLTree<T>::insert(T value) -> void { root = insert(root, value); }

    template <typename T>
    auto AVLTree<T>::remove(T value) -> void { root = remove(root, value); }

    template <typename T>
    auto AVLTree<T>::find(T value) const -> bool { return find(root, value) != nullptr; }

    template <typename T>
    auto AVLTree<T>::getHeight(const std::shared_ptr<TreeNode<T>>& node) -> int32_t { return node ? node->height : 0; }

    template <typename T>
    auto AVLTree<T>::getBalance(const std::shared_ptr<TreeNode<T>>& node) -> int32_t { return node ? getHeight(node->left) - getHeight(node->right) : 0; }
