...
namespace framework::entity::base_type {
    ...
    auto Float::equals(const Float& other) const -> bool { return value_ == other.value_; }

    auto Float::compareTo(const Float& other) const -> int32_t { return (value_ > other.value_) - (value_ < other.value_); }
    ...
    auto Float::parseFloat(const std::string& str) -> Float {
        ...
        catch (const std::invalid_argument&) { throw std::invalid_argument("Invalid input string for Float conversion"); } catch (const std::out_of_range&) { throw std::out_of_range("Value out of range for Float"); }
    }
    ...
    auto Float::operator/(const Float& other) const -> Float {
        if (other.value_ == 0.0f) { throw std::overflow_error("Division by zero"); }
