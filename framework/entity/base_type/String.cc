...
namespace framework::entity::base_type {
    ...
    auto String::length() const -> size_t { return data_.size(); }
    ...
    auto String::concat(const String& str) const -> String { return String(std::string(data_ + str.data_)); }

    auto String::compareTo(const String& anotherString) const -> int32_t { return data_.compare(anotherString.data_); }
    ...
    auto String::isEmpty() const -> bool { return data_.empty(); }

    auto String::contains(const String& str) const -> bool { return data_.find(str.data_) != std::string::npos; }
    ...
    auto String::toStdString() const -> std::string { return data_; }

    auto String::equals(const String& other) const -> bool { return data_ == other.data_; }
    ...
    auto String::matches(const std::string& regex) const -> bool { return std::regex_match(data_, std::regex(regex)); }

    auto String::replaceAll(const std::string& regex, const std::string& replacement) const -> String { return String(std::regex_replace(data_, std::regex(regex), replacement)); }

    auto String::replaceFirst(const std::string& regex, const std::string& replacement) const -> String {
        const std::regex re(regex);
        std::string result = data_;
        if (std::smatch match; std::regex_search(result, match, re)) { result.replace(match.position(0), match.length(0), replacement); }
        return String(result);
    }
    ...
    auto String::split(const std::string& regex) const -> std::vector<String> {
        ...
        while (it != end) { result.emplace_back(*it++); }
        return result;
    }
    ...
    auto String::split(const std::string& regex, const int32_t limit) const -> std::vector<String> {
        ...
        const std::sregex_token_iterator end;
        std::vector<String> result;
        while (it != end && (limit <= 0 || result.size() < static_cast<size_t>(limit - 1))) { result.emplace_back(*it++); }
        if (limit > 0 && it != end) { result.emplace_back(std::string(it->first, data_.end())); }
        return result;
    }
    ...
    auto String::regionMatches(const int32_t tOffset, const String& other, const int32_t oOffset, const int32_t len) const -> bool {
        if (tOffset < 0 || oOffset < 0 || tOffset + len > data_.size() || oOffset + len > other.data_.size()) { return false; }
    ...
    auto String::operator==(const String& str) const -> bool { return equals(str); }
