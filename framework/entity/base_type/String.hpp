...
struct std::formatter<framework::entity::base_type::String> {
  ...
  static auto format(const framework::entity::base_type::String& content, format_context& ctx) -> back_insert_iterator<_Fmt_buffer<char>> { return std::format_to(ctx.out(), "String{{data: {}}}", content.data_); }
};

inline auto operator<<(std::ostream& os, const framework::entity::base_type::String& content) -> std::ostream& { return os << std::format("{}", content); }
