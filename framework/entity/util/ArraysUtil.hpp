...
namespace framework::io {
    ...
    template <typename T>
    auto ArraysUtil::asList(const T* array, size_t size) -> std::vector<T> { return std::vector<T>(array, array + size); }

    template <typename T>
    auto ArraysUtil::binarySearch(const T* array, size_t size, const T& key) -> int32_t {
        auto it = std::lower_bound(array, array + size, key);
        if (it != array + size && *it == key) { return static_cast<int32_t>(it - array); }
        return -1;
    }
    ...
    auto ArraysUtil::binarySearch(const T* array, size_t fromIndex, size_t toIndex, const T& key) -> int32_t {
        ...
        if (it != end && *it == key) { return static_cast<int32_t>(it - array); }
        return -1;
    }
    ...
    template <typename T>
    auto ArraysUtil::fill(T* array, size_t size, const T& value) -> void { std::fill(array, array + size, value); }

    template <typename T>
    auto ArraysUtil::sort(T* array, size_t size) -> void { std::sort(array, array + size); }
