...
namespace framework::io {
    ...
    auto ByteArrayInputStream::read() -> std::byte {
        if (pos_ >= buffer_.size()) { return static_cast<std::byte>(-1); }
        return buffer_[pos_++];
    }
    ...
    size_t ByteArrayInputStream::read(std::vector<std::byte>& cBuf, const size_t off, size_t len) {
        if (off + len > cBuf.size()) { throw std::out_of_range("Offset and length exceed the size of the buffer"); }
        if (const size_t remaining = buffer_.size() - pos_; len > remaining) { len = remaining; }
    ...
    size_t ByteArrayInputStream::available() { return buffer_.size() - pos_; }
