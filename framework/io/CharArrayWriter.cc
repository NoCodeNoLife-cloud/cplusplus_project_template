...
namespace framework::io {
    ...
    CharArrayWriter::CharArrayWriter(const int32_t initialSize) {
        if (initialSize < 0) { throw std::invalid_argument("initialSize must be non-negative"); }
    ...
    void CharArrayWriter::write(const char c) {
        if (count_ == buf_.size()) { buf_.push_back(c); }
        else { buf_[count_] = c; }
        ++count_;
    }

    auto CharArrayWriter::write(const std::vector<char>& cBuf, const size_t off, const size_t len) -> void {
        if (off + len > static_cast<int32_t>(cBuf.size())) { throw std::out_of_range("Invalid offset or length"); }
        if (count_ + len > static_cast<int32_t>(buf_.size())) { buf_.resize(count_ + len); }
    ...
    void CharArrayWriter::write(const std::string& str, const size_t off, const size_t len) {
        if (off + len > static_cast<int32_t>(str.size())) { throw std::out_of_range("Invalid offset or length"); }
        if (count_ + len > static_cast<int32_t>(buf_.size())) { buf_.resize(count_ + len); }
