...
namespace framework::io {
    ...
    auto StringReader::read(std::vector<char>& cBuf, const size_t off, const size_t len) -> size_t {
        if (off >= cBuf.size()) { throw std::invalid_argument("Offset is out of bounds of the buffer"); }
        ...
        {
            if (position_ < source_.size()) { cBuf[off + i] = source_[position_++]; }
            else { break; }
        }
        return actualRead;
    }
    ...
    auto StringReader::reset() -> void {
        if (!mark_set_) { position_ = 0; }
        else { position_ = mark_position_; }
    }
