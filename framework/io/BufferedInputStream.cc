...
namespace framework::io {
    ...
    BufferedInputStream::BufferedInputStream(std::unique_ptr<AbstractInputStream> in, const int32_t size) : FilterInputStream(std::move(in)), buf_(size) {
        if (!&input_stream_) { throw std::invalid_argument("Input stream cannot be null"); }
        if (size <= 0) { throw std::invalid_argument("Buffer size must be greater than zero"); }
    }
    ...
    bool BufferedInputStream::markSupported() const { return true; }

    std::byte BufferedInputStream::read() {
        if (pos_ >= count_)
        {
            fillBuffer();
            if (pos_ >= count_) { return static_cast<std::byte>(-1); }
        }
        return buf_[pos_++];
    }

    size_t BufferedInputStream::read(std::vector<std::byte>& buffer, size_t offset, size_t len) {
        if (offset + len > static_cast<int32_t>(buffer.size())) { throw std::out_of_range("Buffer offset/length out of range"); }
    ...
    auto BufferedInputStream::fillBuffer() -> void {
        if (mark_pos_ < 0 || pos_ - mark_pos_ >= mark_limit_) { mark_pos_ = -1; }
