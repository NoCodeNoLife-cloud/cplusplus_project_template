...
namespace framework::io {
    ...
    std::byte PushbackInputStream::read() {
        if (buffer_pos_ < pushback_buffer_.size()) { return pushback_buffer_[buffer_pos_++]; }
    ...
    size_t PushbackInputStream::read(std::vector<std::byte>& buffer, const size_t offset, const size_t len) {
        if (offset + len > buffer.size()) { throw std::out_of_range("Buffer overflow"); }
        size_t bytesRead = 0;
        while (bytesRead < len && buffer_pos_ < pushback_buffer_.size()) { buffer[offset + bytesRead++] = pushback_buffer_[buffer_pos_++]; }
        if (bytesRead < len) { bytesRead += input_stream_->read(buffer, offset + bytesRead, len - bytesRead); }
        return bytesRead;
    }
    ...
    void PushbackInputStream::unread(const std::vector<std::byte>& buffer, const size_t offset, const size_t len) {
        ...
        for (size_t i = 0; i < len; ++i) { pushback_buffer_[--buffer_pos_] = buffer[offset + len - i - 1]; }
    }
