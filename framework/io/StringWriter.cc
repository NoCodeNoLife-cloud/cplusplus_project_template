...
namespace framework::io {
    ...
    StringWriter& StringWriter::append(const std::string& csq, const size_t start, const size_t end) {
        if (start > end || end > csq.size()) { throw std::out_of_range("Invalid start or end position"); }
    ...
    auto StringWriter::flush() -> void { buffer_.flush(); }

    auto StringWriter::getBuffer() const -> std::string { return buffer_.str(); }

    auto StringWriter::toString() const -> std::string { return buffer_.str(); }

    void StringWriter::write(const char c) { buffer_.put(c); }

    void StringWriter::write(const std::string& str) { buffer_ << str; }

    void StringWriter::write(const std::string& str, const size_t off, const size_t len) {
        if (off > str.size() || off + len > str.size()) { throw std::out_of_range("Invalid offset or length"); }
    ...
    auto StringWriter::write(const std::vector<char>& cBuf, const size_t off, const size_t len) -> void {
        if (off > cBuf.size() || len > cBuf.size() - off) { throw std::out_of_range("Invalid offset or length"); }
