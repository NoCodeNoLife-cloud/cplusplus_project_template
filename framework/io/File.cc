...
namespace framework::io {
    ...
    auto File::createNewFile() const -> bool {
        if (std::filesystem::exists(file_path_)) { return false; }
    ...
    auto File::createTempFile(const std::string& prefix, const std::string& suffix, const std::string& directory) -> File {
        ...
        if (directory.empty()) { GetTempPath(MAX_PATH, tempPath); }
        else { strncpy_s(tempPath, directory.c_str(), MAX_PATH); }
    ...
    auto File::length() const -> int64_t {
        if (std::filesystem::exists(file_path_) && is_regular_file(file_path_)) { return static_cast<int64_t>(file_size(file_path_)); }
        return 0;
    }
    ...
    auto File::list() const -> std::vector<std::string> {
        ...
        if (isDirectory()) { for (const auto& entry : std::filesystem::directory_iterator(file_path_)) { entries.push_back(entry.path().filename().string()); } }
        return entries;
    }
