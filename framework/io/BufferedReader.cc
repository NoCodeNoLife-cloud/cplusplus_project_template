...
namespace framework::io {
    BufferedReader::BufferedReader(std::unique_ptr<AbstractReader> reader, const int32_t size) : reader_(std::move(reader)), buffer_size_(size) {
        if (size <= 0) { throw std::invalid_argument("Buffer size must be greater than 0"); }
        buffer_.resize(size);
    }
    ...
    auto BufferedReader::mark(const size_t readAheadLimit) -> void {
        if (readAheadLimit <= 0) { throw std::invalid_argument("Mark limit must be greater than 0"); }
    ...
    int32_t BufferedReader::read() {
        if (pos_ >= count_) { if (!fillBuffer()) { return -1; } }
        return buffer_[pos_++];
    }

    auto BufferedReader::read(std::vector<char>& cBuf, size_t off, size_t len) -> size_t {
        if (off + len > cBuf.size()) { return -1; }
        size_t totalBytesRead = 0;
        if (len == 0) { return 0; }
        while (len > 0)
        {
            if (pos_ >= count_) { if (!fillBuffer()) { break; } }
    ...
    auto BufferedReader::skip(const int64_t n) -> int64_t {
        if (n <= 0) { throw std::invalid_argument("Skip value must be positive"); }
        int64_t skipped = 0;
        while (skipped < n)
        {
            if (pos_ >= count_) { if (!fillBuffer()) { break; } }
