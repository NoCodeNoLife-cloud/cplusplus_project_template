...
namespace framework::io {
    ...
    CharArrayReader::CharArrayReader(const std::vector<char>& buffer, const size_t offset, const size_t length) {
        if (offset > buffer.size() || offset + length > buffer.size()) { throw std::invalid_argument("Invalid offset or length"); }
    ...
    auto CharArrayReader::read(std::vector<char>& b, const size_t off, const size_t len) -> size_t {
        if (pos_ >= count_) return -1;
        if (off >= b.size() || off + len > b.size()) { throw std::out_of_range("Invalid offset or length for target buffer"); }
    ...
    bool CharArrayReader::ready() const { return pos_ < count_; }

    bool CharArrayReader::markSupported() const { return true; }

    auto CharArrayReader::mark(const size_t readAheadLimit) -> void { marked_pos_ = readAheadLimit; }

    auto CharArrayReader::reset() -> void { pos_ = marked_pos_; }
