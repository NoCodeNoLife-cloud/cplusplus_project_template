...
namespace framework::io {
    FileInputStream::FileInputStream(const std::string& name) {
        if (!std::filesystem::exists(name)) { throw std::ios_base::failure("FileNotFoundException: File does not exist."); }
        if (std::filesystem::is_directory(name)) { throw std::ios_base::failure("FileNotFoundException: Path is a directory."); }
        file_stream_.open(name, std::ios::binary);
        if (!file_stream_.is_open()) { throw std::ios_base::failure("FileNotFoundException: Unable to open file."); }
        file_name_ = name;
    }
    ...
    auto FileInputStream::read() -> std::byte {
        std::byte byte;
        if (file_stream_.read(reinterpret_cast<char*>(&byte), 1)) { return byte; }
    ...
    size_t FileInputStream::read(std::vector<std::byte>& buffer, const size_t offset, const size_t len) {
        if (offset + len > buffer.size()) { throw std::invalid_argument("Invalid buffer, offset, or length."); }
    ...
    auto FileInputStream::close() -> void { if (file_stream_.is_open()) { file_stream_.close(); } }
