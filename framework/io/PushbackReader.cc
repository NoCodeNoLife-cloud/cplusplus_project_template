...
namespace framework::io {
    ...
    PushbackReader::PushbackReader(std::shared_ptr<AbstractReader> reader, const size_t size) : FilterReader(std::move(reader)), buffer_(size) { if (size == 0) { throw std::invalid_argument("Buffer size must be greater than zero."); } }
    ...
    int32_t PushbackReader::read() {
        if (buffer_pos_ < buffer_.size()) { return buffer_[buffer_pos_++]; }
    ...
    size_t PushbackReader::read(std::vector<char>& cBuf, size_t off, const size_t len) {
        ...
        if (bytesRead < len) { bytesRead += FilterReader::read(cBuf, off, len - bytesRead); }
        return bytesRead;
    }
    ...
    size_t PushbackReader::skip(size_t n) {
        ...
        if (n > 0) { skipped += FilterReader::skip(n); }
        return skipped;
    }
    ...
    auto PushbackReader::unread(const std::vector<char>& cBuf, const size_t off, const size_t len) -> void {
        if (len > buffer_pos_) { throw std::overflow_error("Pushback buffer overflow."); }
        for (size_t i = 0; i < len; ++i) { buffer_[--buffer_pos_] = cBuf[off + len - 1 - i]; }
    }

    auto PushbackReader::unread(const int32_t c) -> void {
        if (buffer_pos_ == 0) { throw std::overflow_error("Pushback buffer overflow."); }
