...
namespace framework::io {
    BufferedWriter::BufferedWriter(std::unique_ptr<std::ofstream> os, const size_t size) : output_stream_(std::move(os)), buffer_size_(size) {
        if (!output_stream_->is_open()) { throw std::runtime_error("Output stream is not open."); }
    ...
    void BufferedWriter::write(const std::string& str) {
        ...
        {
            for (const char c : str) { buffer_.push_back(static_cast<char>(c)); }
    ...
    auto BufferedWriter::write(const std::vector<char>& cBuf, const size_t off, const size_t len) -> void {
        if (off + len > cBuf.size()) { throw std::out_of_range("Offset and length are out of the bounds of the buffer."); }
        if (len > buffer_size_)
        {
            flush();
    ...
    auto BufferedWriter::toString() const -> std::string {
        std::string str;
        for (const char c : buffer_) { str.push_back(c); }
        return str;
    }
} // namespace framework::io
