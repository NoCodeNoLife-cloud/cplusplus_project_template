...
namespace framework::io {
    OutputStreamWriter::OutputStreamWriter(std::unique_ptr<AbstractWriter> outputStream, const std::string& charsetName) : output_writer_(std::move(outputStream)), charset_(charsetName), closed_(false) { if (charsetName != "UTF-8") { throw std::invalid_argument("Unsupported encoding: " + charsetName); } }
    ...
    auto OutputStreamWriter::write(const std::vector<char>& cBuf, const size_t off, const size_t len) -> void {
        if (closed_) { throw std::ios_base::failure("Stream is closed"); }
        if (off + len > cBuf.size()) { throw std::out_of_range("Offset and length exceed buffer size"); }
        output_writer_->write(cBuf, off, len);
        if (!output_writer_) { throw std::ios_base::failure("Failed to write to stream"); }
    }

    void OutputStreamWriter::write(const std::vector<char>& cBuf) { write(cBuf, 0, cBuf.size()); }

    void OutputStreamWriter::write(const std::string& str) { write(std::vector<char>(str.begin(), str.end())); }

    void OutputStreamWriter::write(const std::string& str, const size_t off, const size_t len) {
        if (off + len > str.size()) { throw std::out_of_range("Offset and length exceed string size"); }
    ...
    auto OutputStreamWriter::flush() -> void {
        ...
        if (!output_writer_) { throw std::ios_base::failure("Failed to flush stream"); }
    }
