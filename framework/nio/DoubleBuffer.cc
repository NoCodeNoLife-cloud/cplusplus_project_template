...
namespace framework::nio {
    ...
    auto DoubleBuffer::put(const double value) -> DoubleBuffer& {
        if (position_ >= limit_) { throw std::overflow_error("Buffer overflow: Position exceeds limit."); }
        buffer_[position_++] = value;
        return *this;
    }

    auto DoubleBuffer::put(const std::vector<double>& values) -> DoubleBuffer& {
        for (const double value : values) { put(value); }
        return *this;
    }

    auto DoubleBuffer::get() -> double {
        if (position_ >= limit_) { throw std::underflow_error("Buffer underflow: Position exceeds limit."); }
