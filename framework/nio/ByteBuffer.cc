...
namespace framework::nio {
    ...
    void ByteBuffer::put(const std::vector<std::byte>& src) {
        if (src.size() > remaining()) { throw std::overflow_error("Insufficient space in buffer"); }
        std::memcpy(buffer_.data() + position_, src.data(), src.size());
        position_ += src.size();
    }

    void ByteBuffer::put(const std::byte value) {
        if (!hasRemaining()) { throw std::overflow_error("Buffer overflow"); }
        buffer_[position_++] = value;
    }

    auto ByteBuffer::get() -> std::byte {
        if (!hasRemaining()) { throw std::underflow_error("Buffer underflow"); }
        return buffer_[position_++];
    }

    auto ByteBuffer::get(const size_t length) -> std::vector<std::byte> {
        if (length > remaining()) { throw std::underflow_error("Insufficient data in buffer"); }
