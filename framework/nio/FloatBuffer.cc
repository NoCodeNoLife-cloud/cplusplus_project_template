...
namespace framework::nio {
    ...
    auto FloatBuffer::put(const float value) -> void {
        if (position_ >= limit_) { throw std::overflow_error("Buffer overflow"); }
        buffer_[position_++] = value;
    }

    auto FloatBuffer::put(const std::vector<float>& values) -> void {
        if (position_ + values.size() > limit_) { throw std::overflow_error("Buffer overflow"); }
        std::ranges::copy(values, buffer_.begin() + static_cast<std::ptrdiff_t>(position_));
        position_ += values.size();
    }

    auto FloatBuffer::get() -> float {
        if (position_ >= limit_) { throw std::underflow_error("Buffer underflow"); }
        return buffer_[position_++];
    }

    auto FloatBuffer::get(const size_t length) -> std::vector<float> {
        if (position_ + length > limit_) { throw std::underflow_error("Buffer underflow"); }
