...
namespace framework::nio {
    ...
    auto ShortBuffer::get() -> short {
        if (position_ >= limit_) { throw std::out_of_range("Position exceeds limit."); }
        return buffer_[position_++];
    }

    auto ShortBuffer::get(const size_t index) const -> short {
        if (index >= limit_) { throw std::out_of_range("Index exceeds limit."); }
    ...
    auto ShortBuffer::data() -> short* { return buffer_.data(); }

    auto ShortBuffer::data() const -> const short* { return buffer_.data(); }
} // namespace framework::nio
