...
namespace framework::gen {
    ...
    auto RandomGenerator::nextBytes(std::vector<unsigned char>& bytes) -> void {
        ...
        std::ranges::generate(bytes, [&] { return static_cast<unsigned char>(dist(generator)); });
    }

    auto RandomGenerator::nextDouble() -> double { return real_dist_(generator); }

    auto RandomGenerator::nextFloat() -> float { return static_cast<float>(real_dist_(generator)); }

    auto RandomGenerator::nextGaussian() -> double { return normal_dist_(generator); }

    auto RandomGenerator::nextInt() -> int32_t { return next_(32); }
    ...
    auto RandomGenerator::setSeed(const int64_t seed) -> void { generator.seed(seed); }
