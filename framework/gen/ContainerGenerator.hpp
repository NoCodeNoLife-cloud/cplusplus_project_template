...
namespace framework::gen {
    ...
    auto ContainerGenerator::generateVector(T minValue, T maxValue, int32_t size) -> std::vector<T> {
        ...
        std::vector<T> result;
        result.reserve(size);
        for (size_t i = 0; i < size; i++) { result.emplace_back(NextValue<T>(minValue, maxValue)); }
        return result;
    }
    ...
    auto ContainerGenerator::generateForwardList(T minValue, T maxValue, int32_t size) -> std::forward_list<T> {
        ...
        for (size_t i = 0; i < size; i++) { result.emplace_front(NextValue<T>(minValue, maxValue)); }
        return result;
    }
    ...
    auto ContainerGenerator::generateArray(T minValue, T maxValue) -> std::array<T, N> {
        ...
        for (size_t i = 0; i < N; i++) { result[i] = NextValue<T>(minValue, maxValue); }
        return result;
    }
    ...
    auto ContainerGenerator::generateSet(T minValue, T maxValue, int32_t size) -> std::set<T> {
        Check(minValue, maxValue, size);
        std::set<T> result;
        for (size_t i = 0; i < size; i++) { result.emplace(NextValue<T>(minValue, maxValue)); }
        return result;
    }
    ...
    auto ContainerGenerator::generateMap(T minKey, T maxKey, U minValue, U maxValue, int32_t size) -> std::map<T, U> {
        ...
        Check(minValue, maxValue, size);
        std::map<T, U> result;
        for (size_t i = 0; i < size; i++) { result.emplace(NextValue<T>(minKey, maxKey), NextValue<U>(minValue, maxValue)); }
        return result;
    }
    ...
    template <typename T>
    auto ContainerGenerator::Check(T minValue, T maxValue, const int32_t size) -> void { if (minValue > maxValue || size < 0) { throw std::invalid_argument("ContainerGenerator: invalid parameters"); } }
} // namespace framework::gen
