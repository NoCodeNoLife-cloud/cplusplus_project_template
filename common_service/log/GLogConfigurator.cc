...
auto GLogConfigurator::GLogParameters::minLogLevel(const int32_t min_log_level)
    -> void {
  min_log_level_ = min_log_level;
}

auto GLogConfigurator::GLogParameters::logName(const std::string& log_name)
    -> void {
  log_name_ = log_name;
}

auto GLogConfigurator::GLogParameters::logToStderr(const bool log_to_stderr)
    -> void {
...
auto GLogConfigurator::configLogToStdout(const GLogParameters& glog_options)
    -> void {
...
auto YAML::convert<service::GLogConfigurator::GLogParameters>::decode(
    const Node& node, service::GLogConfigurator::GLogParameters& rhs) -> bool {
...
auto YAML::convert<service::GLogConfigurator::GLogParameters>::encode(
    const service::GLogConfigurator::GLogParameters& rhs) -> Node {
  Node node;
