...
TEST(ClassUtilTest, BasicIntType) {
  ...
  EXPECT_EQ(fox::ClassUtil::getTypeId(value), "int");
}
...
TEST(ClassUtilTest, PointerType) {
  int* ptr = nullptr;
  EXPECT_EQ(fox::ClassUtil::getTypeId(ptr), "int * __ptr64");
}

TEST(ClassUtilTest, ConstPointerType) {
  const int* ptr = nullptr;
  EXPECT_EQ(fox::ClassUtil::getTypeId(ptr), "int const * __ptr64");
}

TEST(ClassUtilTest, ReferenceType) {
  constexpr int value = 0;
  const int& ref = value;
  EXPECT_EQ(fox::ClassUtil::getTypeId(ref), "int");
}
...
TEST(ClassUtilTest, ArrayType) {
  int arr[5];
  EXPECT_EQ(fox::ClassUtil::getTypeId(arr), "int [5]");
}
...
TEST(ClassUtilTest, FunctionPointerType) {
  ...
  EXPECT_EQ(fox::ClassUtil::getTypeId(funcPtr), "int (__cdecl*)(int)");
}
