...
namespace gtest_case {
  TEST(StringTest, SplitEmptyString) {
    ...
    EXPECT_EQ(StringUtil::split(target, split_char), expected);
  }
  ...
  TEST(StringTest, ConcatenateEmptyVector) {
    ...
    EXPECT_EQ(StringUtil::concatenate(source, split_char), expected);
  }
  ...
  TEST(StringTest, ConcatenateAfterSplit) {
    ...
    constexpr char split_char = ',';
    const auto parts = StringUtil::split(original, split_char);
