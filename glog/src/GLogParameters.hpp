...
namespace service
{
    ...
    public:
        ...
        inline auto operator==(const GLogParameters& other) const noexcept -> bool;
    ...
    inline auto GLogParameters::deserializedFromYamlFile(const std::filesystem::path& path) -> void
    {
...
inline auto YAML::convert<service::GLogParameters>::decode(const Node& node, service::GLogParameters& rhs) -> bool
{
    if (!node.IsMap())
    {
        return false;
...
inline auto YAML::convert<service::GLogParameters>::encode(const service::GLogParameters& rhs) -> Node
{
    Node node;
